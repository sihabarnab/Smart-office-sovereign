                        ...
                        <table id="data1" class="table table-bordered table-striped table-sm">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-left align-top">Module Name</th>
                                    <th class="text-left align-top">Valid</th>
                                    <th class="text-left align-top"></th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach ($m_module as $key => $value)
                                    <tr>
                                        <form action="{{ route('user_module_update', $emp_id) }}" method="post">
                                            @csrf

                                            <input type="hidden" name="txt_employee_id" id="txt_employee_id" value="{{$emp_id}}">
                                            <td class="text-left align-top">{{ $key + 1 }}</td>
                                            <td class="text-left align-top ">
                                                <input type="hidden" name="txt_module_id" id="txt_module_id" value="{{$value->module_id}}">
                                                {{ $value->module_name }}
                                            </td>
                                            <td class="text-left align-top ">
                                            @php
                                                $user_module = DB::table('pro_module_user')
                                                    ->where('emp_id', $emp_id)
