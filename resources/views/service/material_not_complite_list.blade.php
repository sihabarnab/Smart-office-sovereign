@php
     $req_master = DB::table('pro_service_material_issue_master')
            ->where('status', 1)
            ->where('valid', 1)
            ->get();
@endphp
...
<div class="content-header">
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-sm-6">
                <h1 class="m-0">Material issue not complite list</h1>
            </div><!-- /.col -->
        </div><!-- /.row -->
    </div><!-- /.container-fluid -->
</div>
                    ...
                    <table id="data1" class="table table-bordered table-striped table-sm">
                        ...
                        <tbody>
                            @foreach ($req_master as $key => $row_req_master)
                                ...
                                <tr>
                                    <td>{{ $key + 1 }}</td>
                                    <td>{{ $row_req_master->smi_master_id }}<br>{{ $row_req_master->entry_date }}</td>
                                    ...
                                    <td>
                                        <a
                                            href="{{ route('material_issue_details', $row_req_master->smi_master_id) }}">Next</a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
