...
@section('content')
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">Material Issue List</h1>
                </div><!-- /.col -->
            </div><!-- /.row -->
        </div><!-- /.container-fluid -->
    </div>
                                    ...
                                    @foreach ($m_requisition_master as $key => $row_req_master)
                                        @php
                                            ...
                                            if ($row_req_master->approved_id) {
                                                $approved = DB::table('pro_employee_info')
                                                    ->Where('employee_id', $row_req_master->approved_id)
                                                    ->first();
                                                $approvedby = $approved->employee_name;
                                            } else {
                                                $approvedby = 'Pending';
                                            }
                                            if ($row_req_master->final_approved_id) {
                                                $approvedMGM = DB::table('pro_employee_info')
                                                    ->Where('employee_id', $row_req_master->final_approved_id)
                                                    ->first();
                                        ...
                                        <tr>
                                            <td>{{ $key + 1 }}</td>
                                            <td>{{ $row_req_master->requisition_master_id }}<br>{{ $row_req_master->entry_date }}
                                            </td>
                                            <td>{{ $row_req_master->complain_id }}<br>{{ $txt_complain_date }}<br>{{ $txt_complaint_description }}
                                            </td>
                                            ...
                                            <td>
                                                <a
                                                    href="{{ route('material_issue_create', $row_req_master->requisition_master_id) }}">Create</a>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
