                ...
                <div class="card-body">
                    <table id="data1" class="table table-border table-striped table-sm">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>{{ $key+1 }}</td>
                                <td>{{ $txt_company_name }}<BR>{{ $row->employee_id }}<BR>{{ $row->employee_name }}</td>
                                <td>{{ $txt_desig_name }}<BR>{{ $txt_department_name }}<BR>{{ $txt_placeofposting_name }}</td>
                                <td>{{ $txt_joinning_date }}/{{ $txt_att_policy_name }}/{{ $txt_yesno_name }}<BR>{{ $row->mobile }}/{{ $txt_blood_name }}<BR>{{ $row->psm_id }}</td>
                                <td>{{ $txt_report_name }}</td>
                                <td>
                                    <a href="{{ route('hrmbackbasic_infoedit',$row->employee_info_id) }}" class="btn btn-info btn-sm"><i class="fas fa-edit"></i></a>
                                </td>
                            </tr>
                            @endforeach
